...
/* Repositorios que no quiero que salgan en la lista. En minúsculas, porque
   más abajo se compara así y no depende de cómo estén escritos en GitHub.
   Para ocultar otro, basta con añadir su nombre aquí. */
const REPOS_OCULTOS = [
  "adanprog",         // el repositorio de esta misma web
  "ballerlink-legal", // páginas legales de BallerLink
];

// Cuántos proyectos destacados hay escritos a mano encima de la lista
const destacados = document.querySelectorAll(".projectList--featured .projectRow").length;

// Construye el HTML de las filas y lo mete en la página
const renderProjects = (repos) => {
  projectsGrid.innerHTML = repos
    .map((repo, i) => {
      const color = LANG_COLORS[repo.language] || "#38bdf8";

      // Hay repositorios sin lenguaje detectado; en esos no pintamos el puntito
      const language = repo.language
        ? `<span class="projectRow__dot" style="background:${color}"></span>${escapeHtml(repo.language)} ·`
        : "";

      /* La numeración continúa después de los destacados que van escritos
         a mano en el HTML. Se cuentan en vez de poner el número fijo, para
         que siga cuadrando si algún día se añade o se quita alguno. */
      const numero = String(i + 1 + destacados).padStart(2, "0");

      /* Las comillas invertidas (`) permiten escribir varias líneas y meter
         valores dentro con ${...}. Se llaman plantillas literales. */
      return `
        <li class="projectRow spot">
          <a href="${escapeHtml(repo.html_url)}" target="_blank" rel="noreferrer">
            <span class="projectRow__num">${numero}</span>
            <span class="projectRow__body">
              <span class="projectRow__name">${escapeHtml(repo.name)}</span>
              <span class="projectRow__desc">${escapeHtml(repo.description || "Sin descripción todavía.")}</span>
            </span>
            <span class="projectRow__meta">${language} ${formatDate(repo.pushed_at)}</span>
            <svg class="projectRow__arrow" viewBox="0 0 24 24" aria-hidden="true"><path d="M13.3 5.3 12 6.7l4.3 4.3H4v2h12.3L12 17.3l1.3 1.4 6.7-6.7-6.7-6.7Z"/></svg>
          </a>
        </li>`;
    })
...
const renderProjectsFallback = () => {
  ...
  projectsGrid.innerHTML = `
    <p class="emptyState">
      ...
      <a href="https://github.com/adanprog?tab=repositories" target="_blank" rel="noreferrer">github.com/adanprog</a>.
    </p>`;
};
...
if (projectsGrid) {
  ...
  fetch("https://api.github.com/users/adanprog/repos?sort=pushed&per_page=100")
    ...
    .then((repos) => {
      const visible = repos
        // Fuera los forks y los archivados
        .filter((repo) => !repo.fork && !repo.archived)
        // ...y los que no quiero enseñar, por nombre
        .filter((repo) => !REPOS_OCULTOS.includes(repo.name.toLowerCase()))
